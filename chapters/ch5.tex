...
\[
    \mathbb{E}[X^2]=\sum_{n=1}^{\infty} \mathbb{E}[\abs{J_k X}^2].
\]
...
下面将到导数算子的定义域从Srv"延拓"到更大的集合上.为此,先介绍以下概念.(见\citep{Yosida}第二章第6节)\par
\begin{definition}
...
\begin{proof}
    ...
    \]
    且当$a,b\in \Lambda,a\ne b$,$D\Phi_a和D\Phi_b$在$L^2(\Omega;\mathbb{H})$中是正交的,即$\mathbb{E}[\ipr{D\Phi_a}{D\Phi_b}]=0$.
...
\begin{remark}
    ...
    可积性条件.
    其中一个原因是,经典微积分中的导数定义是局部的,固定一点处的导数只取决于局部性态,而太远处的行为则影响不大,但Malliavin导数则不然,它依赖于全空间上的积分.
\end{remark}
...
\begin{lemma}
    ...
    \[
        \mathbb{E}(F| \mathscr{F}_A)=\sum_{n=0}^{\infty}I_n(f_n(I_A)^{\otimes n})
    \]\footnote{$I_0=\mathrm{id}_{\mathbb{R}}$}
\end{lemma}
由以上引理可得以下命题.
\begin{proposition}
...
$\mathbb{L}^{1,2}\subset \dm{\delta}$.若$u,v\in \mathbb{L}^{1,2}$,则
...
\end{align*}
为可测映射,分别称为漂移系数和扩散系数.$\sigma$作为一个矩阵值映射,记其第$j$列为$A^j$,所以方程\ref{sde1}还可以写为
...
为了与第二节中的记号对应,取$\Omega=C_0([0,T];\mathbb{R}^d)$为$[0,1]$上在$0$处取值为$0$的$\mathbb{R}^d$值连续函数全体,$\mathbb{P}$为Wiener测度,
...
先陈述本节的主要定理如下.
\begin{theorem}[SDE解的Malliavin可微性]
    ...
    \]
    且存在一致有界的$m$维适应过程$\bar{A}^{k,l}_s,\bar{B}^k_s$,使得
    \[
        ***
    \]
\end{theorem}
