...
下面将Ito随机积分作为Skorohod随机积分的特例来讨论.\par
...
对适应过程$u\in \mathbb{L}^{1,2}$,$u_t$是$\mathscr{F}_t$可测的,所以由\ref{lab3}可得$D_su_t=0,s>t$,所以对易关系蕴含Ito等距:
\[
...
\begin{proof}[定理\ref{SDE解的Malliavin可微性}的证明]
   ...
   由于"被积函数"$D\sigma^{i,j}_s(X_n(s))\in \mathbb{D}^{1,2}$,所以Ito随机积分
   \[
