...
\begin{definition}
    ...
    (2)\ $\overline{\text{span}(\cup_{n\geq 1}E_n)}=\mathbb{H}$.\par
...
\begin{proof}[定理\ref{SDE解的Malliavin可微性}的证明]
   ...
   \begin{align*}
    &D\sigma^{i,j}_s(X_n(s))=\sum_{l=1}^{m}\bar{A}^{n,i,j,l}_sDX_n^i(s)\in \mathbb{D}^{1,\infty},\\
    &Db^i_s(X_n(s))=\sum_{l=1}^{n}\bar{B}^{n,i,l}_s DX_n^i(s)\in \mathbb{D}^{1,\infty}.
   \end{align*}\par
   ...
   \]\par
   对于随机积分,由于“被积函数”$D\sigma^{i,j}_s(X_n(s))\in \mathbb{D}^{1,2}$,所以It\^{o}随机积分
   \[
   ...
   \]
   这个等式说明了随机积分项的可导性,下面说明可积性由BDG不等式得到:
   \begin{align*}
   ...
   \end{align*}
    这样就得到了$X_{n+1}(t)\in \mathbb{D}^{1,\infty}$.
   下面估计$\mathbb{E}[\sup_{r\leq s\leq T}\abs{D_rX_{n+1}(s)}^p], p\geq 2$.
   \begin{align*}
   ...
   根据\ref{labint},可得$X(t)\in L^p(\Omega),\forall p\geq 2$,也就完成了证明.
\end{proof}
\begin{remark}
    若系数是连续可微且导数有界,则$\bar{A}_{k,l},\bar{B}_k$就是对应的偏导数.\par
    在SDE的一阶可导性中,一次连续可微和Lipschitz连续的差别并不大,因为可以用磨光子逼近.有了一阶可导,自然会关心更高阶的可导性或者说光滑性.
    直觉上需要提高系数的光滑性,实际上SDE的解任意次Malliavin可导的一个充分条件就是系数是光滑的且各阶导数有界,证明的大概思路还是使用归纳法,
    只是不再是对Picard迭代做归纳,而是对导数阶数做归纳,具体可见\citep{Nua1}的定理2.2.2.
\end{remark}
